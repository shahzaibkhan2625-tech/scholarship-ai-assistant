...
- [ ] T130 [P] Run `specs/001-scholarship-mvp/quickstart.md` validation against all Phase 1–4 endpoints — Verified live end-to-end (docs/t130-verification-log.md); 2 reproducible bugs found (Qdrant timeout, missing payload index) + Phase 2 discovery gap flagged (web-access-method sources return zero results, undocumented as a deferred decision). Check only after bugs are fixed and re-verified. Bugs 1+2 (Qdrant timeout, missing payload index) fixed in commit 32a47f5 and re-verified live (docs/t130-verification-log.md, "Bugs 1+2 re-verified" section). Remaining open item before checking this box: Phase 2 discovery gap (web-access-method sources return zero results) — still undocumented as a deferred decision, needs a product decision before T130 can be marked fully complete.
- [X] T131 [P] Code-review checklist pass: confirm no LLM code path reaches into `hard_constraints`/`ranking` (Principle II) and no code path calls a live external submission API (Principle III) — constitution "Development Workflow & Quality Gates" — PASS — see full trace in this session; no violations found, 29/29 relevant tests green.
- [X] T132 [P] Security hardening pass: confirm every repository query is `user_id`-scoped (FR-AUTH-2) across `profile_repo`, `scholarship_repo`, `match_repo`, `source_repo`, `document_repo`, `application_repo` — PASS — 30 functions reviewed, all correctly scoped or justified as global; one defense-in-depth gap found and hardened (profile_repo's six object-based mutators now independently verify ownership, matching document_repo's pattern).
- [X] T133 Regenerate/verify `specs/001-scholarship-mvp/contracts/openapi.yaml` matches implemented FastAPI routes (drift check) — PASS — fixed #15 (real bug: 422 response schema now correctly documents FetchFailure), added missing responses= declarations for 7 domain-specific status codes, updated openapi.yaml to match live schema for all 18 originally-flagged items, zero remaining drift verified via 34-point targeted check. New unrelated contract gaps found by stricter tooling logged as follow-up task T136.
...
- [X] T136 Fix OpenAPI contract gaps surfaced by T133's stricter drift check (missing required arrays on MatchVerdict, QaAnswer, TokenResponse, UserResponse, GeneratedDocument, ApplicationPlan, AssistantStepResult, DiscoveryResult; enum-vs-bare-string inlining) — PASS — allOf-aware diff found 18 schemas missing `required` (10 more than the 8 originally named: also UserResponse, ProfileCriterion, MissingInfoItem, Profile, UrlMatchResult, FetchFailure, CriterionOutcome, CoverageSummary, DiscoveryResultItem, UploadedDocument, ChecklistItem), all fixed from live schema's required set; CoverageSummary was also missing 3 properties entirely (as_of, sources_configured, sources_active), added typed to match live; enum-vs-bare-string was a single genuine gap (ProfileCriterion.dimension), not several — SourceRegistryCreate and 11 other enum-backed properties already matched. Zero remaining drift verified via 22-point targeted check (18 required-array + 1 enum + 3 new-property type checks).
...
- [X] T139 Replace deprecated HTTP_422_UNPROCESSABLE_ENTITY with HTTP_422_UNPROCESSABLE_CONTENT (StarletteDeprecationWarning, 4 call sites surfaced by the CI run at commit 65cee1f).
